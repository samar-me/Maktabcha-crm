...
import { Homework, Student, HomeworkSubmission, HomeworkStatus } from "@/types/database";
...
import { Check, CheckCircle2, AlertCircle, XCircle, Clock, Save } from "lucide-react";
...
export function HomeworkGradingDialog({
  ...
  React.useEffect(() => {
    ...
    const existingSubmissions = crmStore.getHomeworkSubmissions(homework.id);
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      crmStore.saveHomeworkSubmissionBatch(
        homework.id,
        rows.map((r) => ({
